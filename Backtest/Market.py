...
class Market:
	...
	def orderCreate(self,action,lot,stopLoss,takeProfit):
		...
		self.openArray.append([self.marketTick,self.getCurrentMarketData()])
		return orderID
	...
	def getPrevMarketData(self,num):
		if (self.marketTick<num):
			num=self.marketTick
		return self.data[self.marketTick-num:self.marketTick]
	...
	def deleteClosedOrder(self,order):
		self.closeArray.append([self.marketTick,self.getCurrentMarketData()])
